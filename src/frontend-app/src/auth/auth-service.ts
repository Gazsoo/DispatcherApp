...
export class AuthService {
  static async login(email: string, password: string): Promise<void> {
    ...
    const authResponse = await apiClient.auth_Login(request);
    JWTManager.setTokens(authResponse);
  }

  static async register(email: string, password: string, firstName?: string, lastName?: string, role?: string): Promise<void> {
    const request: RegisterRequest = { email, password, firstName, lastName, role };
